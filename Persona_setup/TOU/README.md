...
TOU-family persona (TOU, TOU Promotion, or TOU Coaching), by writing exactly
the configs the
...
This is not the TOU **onboarding email** flow - see
[`../TOUOnboarding/`](../TOUOnboarding/README.md) for that. This script just
sets the persona's rate plan and Monthly Summary email variant.
...
3. **Persona config** - writes `email_moderation.monthly_summary_email_variant`
...
| `VARIANT` | Persona | Rate plan # | `monthly_summary_email_variant` |
|---|---|---|---|
| `PLAIN` | TOU | 24 (`608A`) | `MS_STANDARD` |
| `PROMOTION` | TOU Promotion | 1 (`180`, non-TOU) | `MS_TOU_PROMOTION` |
| `COACHING` | TOU Coaching | 24 (`608A`) | `MS_TOU_COACHING` |

TOU Promotion intentionally stays on a **non-TOU** plan - it's a promotional
persona meant to be shown a TOU upsell, not already on one.
...
`"scripts": ["TOU"]`, plus any per-script setting (e.g. `VARIANT`).

Nothing about the pilot is hardcoded: point `BASE_URL` and `PILOT_ID` at a new
pilot and this script works there unchanged.

These may be set on the user entry, under `scripts.TOU`, or left at the
default:
...
|-----|---------|----------|
| `VARIANT` | `PLAIN` | `PLAIN`, `PROMOTION`, or `COACHING` (see table above). |
...
```bash
uv run python setup_tou_persona.py
```
...
- **`user_tou_status`** - disagg-computed data (`user_tou_status_data`), not
  a config. Being on a TOU-shaped rate plan doesn't by itself make the
  platform treat the user as "on TOU" for every downstream feature; that
  flag is set by disagg, not by this script.
...
- **`monthly_summary_email_variant` is an unverified key name.** It is not
  ...
  keys, but confirm the email actually renders with the right variant before
  relying on it.
